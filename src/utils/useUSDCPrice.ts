...
export default function useUSDCPrice(currency?: Currency): Price | undefined {
  ...
  const tokenPairs: [Currency | undefined, Currency | undefined][] = useMemo(
    ...
    [chainId, wrapped, daiToken, oldQuickToken, usdtToken],
  );
  ...
  //   }
  //   // handle usdc
  //   if (wrapped.equals(usdtToken)) {
  //     return new Price(usdtToken, usdtToken, '1', '1');
  //   }
  ...
  //     usdcPair &&
  //     usdcPair.reserveOf(usdtToken).greaterThan(ethPairETHUSDCValue)
  //   ) {
  ...
  //   ) {
  //     if (
  //       usdcEthPair.reserveOf(usdtToken).greaterThan('0') &&
  //       ethPair.reserveOf(WETH[chainId]).greaterThan('1')
  //     ) {
  //       const ethUsdcPrice = usdcEthPair.priceOf(usdtToken);
  ...
  //   ) {
  //     if (
  //       usdcQuickPair.reserveOf(usdtToken).greaterThan('0') &&
  //       quickPair.reserveOf(oldQuickToken).greaterThan('5')
  //     ) {
  //       const quickUsdcPrice = usdcQuickPair.priceOf(usdtToken);
...
export function useUSDCPricesToken(tokens: Token[]) {
  ...
  const usdtToken = GlobalValue.tokens.COMMON.USDT;
  const [, quickUsdcPair] = usePair(oldQuickToken, usdtToken);
  // const [, newQuickUsdcPair] = usePair(newQuickToken, usdtToken);
